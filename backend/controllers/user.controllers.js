...
import { getScocketId } from "../socket.js";
...
export const suggestUsers = async (req, res) => {
    try {
        ...
        return res.status(500).json({ message: `suggest users error ${error}` });
    }
}
...
export const editProfile = async (req,res) => {
    try {
        ...
        if(req.file) {
            ...
            profileImage = user.profileImage; // Keep the existing profile image if no new one is uploaded
        }
    ...
    try {
        ...
        if(isFollowing){
            ...
            if(currentUser._id!=targetUser._id){
                            ...
                            const receiverSocketId = getScocketId(targetUser._id)
                            if(receiverSocketId){
...
export const getAllNotifications = async (req,res) => {
    try {
        ...
        return res.status(500).json({ message: `get allNotifications error ${error}` });
    }
}
