...
export const uploadPost = async (req,res) => {
    try {
        ...
        return res.status(500).json({ message: `Error uploading post: ${error.message}` });    
    }
}
    ...
    try {
        ...
        if(alreadyLiked) {
            ...
            if(post.author._id!=req.userId){
                ...
                const receiverSocketId = getScocketId(post.author._id)
                if(receiverSocketId){
        ...
        return res.status(500).json({ message: `Error liking post: ${error.message}` });
    }
}
...
export const comment = async (req,res) => {
    try {
        ...
        return res.status(500).json({ message: `Error commenting on post: ${error.message}` });
    }
}
...
export const saved = async (req,res) => {
    try {
        ...
        return res.status(500).json({ message: `Error saving post: ${error.message}` });
    }
}