...
const StoryDp = ({profileImage,userName,story}) => {
  ...
  const handleViewers = async () => {
    try {
      ...
      const result = await axios.get(`${serverUrl}/api/story/view/${story}`,{withCredentials:true})
    } catch (error) {
      console.log(error);
    }
  }
  ...
  return (
    <div className='flex flex-col w-[80px]'>
        <div className={`w-[80px] h-[80px] ${!story ? null : !viewed ? "bg-gradient-to-b from-blue-500 to-blue-950" : "bg-gradient-to-r from-gray-500 to-black-800" } rounded-full flex justify-center relative items-center`} onClick={handleClick}>
